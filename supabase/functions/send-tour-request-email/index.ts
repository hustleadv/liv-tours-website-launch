...
const generateTourRequestEmailHtml = (data: TourRequestEmailData): string => {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
          ...
          <tr>
            <td style="padding: 30px;">
              <div style="background: linear-gradient(135deg, #e8a54b 0%, #d4922e 100%); border-radius: 12px; padding: 20px; text-align: center; margin-bottom: 25px;">
                <p style="margin: 0 0 5px 0; color: rgba(255,255,255,0.9); font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">Request ID</p>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                  <tr>
                    ...
                    <td style="padding: 12px 0; border-bottom: 1px solid #e9ecef; text-align: right;">
                      <span style="color: #212529; font-weight: 600;">${data.tourTitle}</span>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding: 12px 0; border-bottom: 1px solid #e9ecef;">
                  ...
                  <tr>
                    <td style="padding: 12px 0; border-bottom: 1px solid #e9ecef;">
                      <span style="color: #6c757d; font-size: 14px;">🕐 Preferred Time</span>
                    </td>
                    <td style="padding: 12px 0; border-bottom: 1px solid #e9ecef; text-align: right;">
                      <span style="color: #e8a54b; font-weight: 700;">${data.preferredTime}</span>
                    </td>
                  </tr>
                  ` : ''}
                  <tr>
                    <td style="padding: 12px 0; border-bottom: 1px solid #e9ecef;">
                      <span style="color: #6c757d; font-size: 14px;">📍 Pickup Area</span>
                    </td>
                    <td style="padding: 12px 0; border-bottom: 1px solid #e9ecef; text-align: right;">
                  ...
                  <tr>
                    ...
                    <td style="padding: 12px 0; text-align: right;">
                      <span style="color: #212529; font-weight: 600;">${data.groupSize}</span>
                    </td>
                  </tr>
                </table>
              </div>
              ...
              <div style="background: #fef3c7; border-radius: 12px; padding: 20px; margin-bottom: 25px; border: 1px solid #fcd34d;">
                <h2 style="margin: 0 0 10px 0; color: #92400e; font-size: 16px; font-weight: 600;">💵 Payment Method</h2>
                <p style="margin: 0; color: #78350f; font-size: 14px;">Pay on Arrival - You'll pay the driver directly on the day of the tour</p>
              </div>
              ...
              <div style="background: #fffbeb; border-radius: 12px; padding: 20px; border: 1px solid #fcd34d;">
                ...
                <ul style="margin: 0; padding: 0 0 0 20px; color: #78350f; font-size: 14px; line-height: 1.8;">
                  ...
                  <li>You'll receive a confirmation via WhatsApp or email within 24 hours</li>
                  <li>Your driver will contact you 24 hours before the tour</li>
                  <li>Payment is made directly to the driver on the day</li>
                </ul>
              </div>
            </td>
          </tr>
...
const handler = async (req: Request): Promise<Response> => {
  console.log("[TOUR-REQUEST-EMAIL] Received request");
  ...
  try {
    const data: TourRequestEmailData = await req.json();
    
    console.log("[TOUR-REQUEST-EMAIL] Sending to:", data.customerEmail);
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `📥 Tour Request Received - ${data.requestId} | LIV Tours`,
        html: generateTourRequestEmailHtml(data),
      }),
    });

    const emailResult = await emailResponse.json();
    console.log("[TOUR-REQUEST-EMAIL] Response:", emailResult);
